...
output "grpc" {
  value = "http://${data.kubernetes_service.prometheus.status[0].load_balancer[0].ingress[0].hostname}/graph?g0.expr=sum(rate(grpc_server_msg_received_total%7Bgrpc_type!%3D%22unary%22%7D%5B5m%5D))%20by%20(grpc_service%2C%20grpc_method)&g0.tab=0&g0.display_mode=lines&g0.show_exemplars=0&g0.range_input=1h&g1.expr=histogram_quantile(0.95%2C%20sum(rate(grpc_server_handling_seconds_bucket%7Bgrpc_type!%3D%22unary%22%7D%5B5m%5D))%20by%20(grpc_service%2Cgrpc_method%2C%20le))&g1.tab=0&g1.display_mode=lines&g1.show_exemplars=0&g1.range_input=1h&g2.expr=grpc_server_started_total&g2.tab=1&g2.display_mode=lines&g2.show_exemplars=0&g2.range_input=1h"
}
